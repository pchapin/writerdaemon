 ...
 * text.
 */
class SourceText(private val paragraphs: Seq[Paragraph]) extends Traversable[Paragraph] {
   ...
   */
  def foreach[U](f: Paragraph => U): Unit = paragraphs foreach f

  /**
   * @return The number of paragraphs in the source text.
   */
  override def size = paragraphs.size
}
