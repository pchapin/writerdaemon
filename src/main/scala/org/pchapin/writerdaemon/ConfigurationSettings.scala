...
class ConfigurationSettings(private val configurableItems: Map[String, String => String]) {
   ...
   * The value can contain any text, but the quotation marks are required in all cases. There
   * can be only one (name, value) pair on each line. Blank lines are ignored. Leading white
   * space, trailing white space, and white space around the '=' character are all ignored.
   * Whitespace inside the value string is part of the value. Comments start with a '#'
   * character and go to the end of the line.
   *
   * @param fileName The name of the file to open. If the file cannot be opened, there is no
...
object ConfigurationSettings {
   ...
   * readable, etc. In some applications it is not an error to configure the name of a
